...
The icon can be customized in the `appfair.xcconfig` configuration file
with the keys:
...
In addition to the canonical `App-Name` name, this name must be mirrored in the app's `appfair.xcconfig` metadata file.
...
values from the `appfair.xcconfig` configuration file,
...
Create a new GitHub Organization with the new name, and then fork your previous organization's app into the new organization. Change the `appfair.xcconfig` metadata `PRODUCT_NAME` and `PRODUCT_BUNDLE_IDENTIFIER` keys to reflect the new name.
...
Once the `appfair.xcconfig` metadata is updated to reflect the new name, releases can be issued from the new app.
...
The app's name is specified by the `PRODUCT_NAME` and `PRODUCT_BUNDLE_IDENTIFIER` keys in your fork's `appfair.xcconfig` metadata file.
...
These settings should also match the `APP_CATEGORY` property in your `appfair.xcconfig` file, with the following GitHub topic property mapping:
...
`appfair.xcconfig` can be edited to remove the unsupported platform
...
the `appfair.xcconfig` to the new name.
...
You can simply bump the semantic release `MARKETING_VERSION` property of your `appfair.xcconfig` and push the new tag, and the next version of your app will be up-to-date with your latest dependencies.
...
### Metadata
  ...
  1. Is your `appfair.xcconfig` `PRODUCT_NAME` property set to `App Name` (with a space)?
  1. Is your `appfair.xcconfig` `PRODUCT_BUNDLE_IDENTIFIER` property set to `app.App-Name` (with a hyphen)?
  1. Is your `appfair.xcconfig` `APP_CATEGORY` property set to a valid category?
  1. Is your `appfair.xcconfig` `MARKETING_VERSION` property set to a semantic release number that exactly matches your intended release tag?
...
### Pull Request
  ...
  1. Does the version tag of your PR match the version in `appfair.xcconfig`?
